...
- Increasing demand for agricultural  productions, combined with land restrictions and changing weather has contributed to the extensive use of agrochemicals
            
- Issue:This use of chemicals has severe negative effects on the populations living close to the farms or working on them
            
* But, still there are few studies analyzing the effects of environmental pollution on health outcomes
...
- Use of mother's address during pregnancy and precise information on the perimeter of the plantations to compute measures of exposure to air pollution
...
$$
 Y_{ijmy} =\beta_{0} +\beta_{1}\text{Banana Exposure}_{ijmy} +\sum_{z=1}^{3}\beta_{z} Z^{th}\text{Intensive Fumigations}_{ijmy}\\ +\sum_{z=1}^{3}\theta_{z}[ \text{Banana Exposure}_{ijmy} \times   Z^{th}\text{Intensive Fumigations}_{ijmy}] + \\   \delta X_{i} + \mu_{j} + \psi_{m} + \pi_{y} + \epsilon_{ijmy} 
$$
...
- $\text{Banana Exposure}_{ijmy}$: is a dummy variable that takes the value 1 for newborns from mothers geographically exposed to pesticides
 - $\text{Intensive Fumigations}_{ijmy}$: is a dummy variable that takes the value 1 for newborns that were affected by intensive fumigations for at least three months during gestation
- $Z^{th}\text{Intensive Fumigations}_{ijmy}$: is a dummy variable that takes the value 1 for the newborns that were affected by intensive fumigations during the three months of the $Z^{th}$ gestation trimester.
...
estimate_coefs = function(data, dependent_var, independent_var, fixed_effects, remove_var, cluster_var, model_type) {
    boot_data = boot_dataset(data, cluster_var) %>% tibble() # Make the boot dataset that will be used for one estimation
