...
set_model <- function(data, dependent_var, independent_var, fixed_effects, model_type) {
    ...
    mom_controls <- paste(c("age", "educ", "private", "labors", "nchild", "visits", "nbl", "sex", "tlabor", "marital", "ethnic"), collapse = " + ") #Every rergression on the paper use these controls
...
clusterbootreg_boot <- function(formula, data, cluster, reps, model_type, model_number) {
    ...
    if (model_type == "ols") {
        ...
        reg_fit <- glm(formula, data, family = binomial)
    }
